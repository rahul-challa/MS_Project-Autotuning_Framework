...
def run_sequential_autotuning(
    ...
    if selection_strategy not in {"epsilon_greedy", "deterministic_sweep"}:
        raise ValueError(
            "selection_strategy must be one of {'epsilon_greedy', 'deterministic_sweep'}"
        )
    if random_restarts < 1:
        raise ValueError("random_restarts must be >= 1")
    if init_strategy not in {"middle", "random"}:
        raise ValueError("init_strategy must be one of {'middle', 'random'}")
    ...
    def _init_config() -> Dict[str, int]:
        cfg: Dict[str, int] = {}
        for p, values in TUNABLE_PARAMETERS.items():
            if init_strategy == "random":
                cfg[p] = int(np.random.choice(values))
            else:
                cfg[p] = values[len(values) // 2]
        return cfg

    # Global tracking across restarts
    ...
    for restart_idx in range(1, random_restarts + 1):
        print("\n" + "=" * 70)
        print(f"RESTART {restart_idx}/{random_restarts} (init_strategy={init_strategy})")
        print("=" * 70)

        current_config = _init_config()
        ...
        for round_num in range(1, num_rounds + 1):
            print("\n" + "=" * 70)
            print(f"ROUND {round_num}/{num_rounds} (Restart {restart_idx})")
            print("=" * 70)
            ...
            for param_idx, param_name in enumerate(param_order):
                print("=" * 70)
                print(
                    f"TUNING PARAMETER {param_idx + 1}/{len(param_order)}: {param_name} "
                    f"(Round {round_num}, Restart {restart_idx})"
                )
                print("=" * 70)
                print(f"Possible values: {TUNABLE_PARAMETERS[param_name]}")
                print(f"Current value: {current_config[param_name]}")
                print(
                    f"Other parameters fixed at: "
                    f"{[f'{k}={v}' for k, v in current_config.items() if k != param_name]}"
                )
                print()
                ...
                # Choose which values to test for this parameter
                if selection_strategy == "deterministic_sweep":
                    values_to_test = list(param_values)
                else:
                    values_to_test = []
                    for iteration in range(iterations_per_param):
                        if iteration < len(param_values):
                            values_to_test.append(param_values[iteration])
                        else:
                            if np.random.random() < exploration_rate:
                                values_to_test.append(np.random.choice(param_values))
                            else:
                                values_to_test.append(param_best_value)

                # Test selected values for this parameter
                for iteration, test_value in enumerate(values_to_test):
                ...
                print(
                    f"\nBest value for {param_name} (Round {round_num}): "
                    f"{param_best_value} (error: {param_best_error:.6f})"
                )
            ...
            print(f"Best error after round {round_num}: {best_error:.6f}")
